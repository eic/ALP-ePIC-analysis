...
void reserve_bdt_histograms(map<string,TH1*>& h1, map<string,TH2*>& h2, map<string,TH3*>& h3); //AW 20260608
...
void MyAnalysis::ReserveHistograms()
{
    ...
    reserve_bdt_histograms(h1, h2, h3); // AW 20260608
...
void reserve_bdt_histograms(map<string,TH1*>& h1, map<string,TH2*>& h2, map<string,TH3*>& h3){
    //AW 20260608
    //declare the histograms here that I want!!!!
    ...
    //h1["dis_elec_pt_miss"] = new TH1F("dis__elec_pt_miss","Missing p_{T}",pTbins.size() - 1, &pTbins[0]); //etc etc etc
    ...
    //h1["dis_elec_sig_label"] = new TH1F("bdt_sig_label","Signal or Background label",4,-1,2); //etc etc etc
    ...
    h1["dis_e"] = new TH1F("dis_elec_e","Energy of the Electron or highest p_{T} particle",pTbins.size() - 1, &pTbins[0]); //etc etc etc
    h1["dis_pt"] = new TH1F("dis_elec_pt","Transverse Momentum p_{T}",pTbins.size() - 1, &pTbins[0]); //etc etc etc
    h1["dis_eta"] = new TH1F("dis_elec_eta","Pseudorapidity #eta",etabins.size() - 1, &etabins[0]); //etc etc etc
    //h1["dis_elec_beam_rat"] = new TH1F("dis_elec_beam_rat","Energy_{e}/Energy{beam} ratio",100,-5,5); //etc etc etc
    //h1["dis__elec_pt_miss"] = new TH1F("dis__elec_pt_miss","Missing p_{T}",pTbins.size() - 1, &pTbins[0]); //etc etc etc
    h1["dis_p"] = new TH1F("dis_elec_p","Momentum of the Electron p_{e}",pbins.size() - 1, &pbins[0]); //etc etc etc
    h1["dis_phi"] = new TH1F("dis_elec_phi","Phi",phibins.size() - 1, &phibins[0]); //etc etc etc
    //h1["dis_elec_sig_label"] = new TH1F("bdt_sig_label","Signal or Background label",4,-1,2); //etc etc etc
    //h1["dis_elec_q2"] = new TH1F("dis_elec_q2","Q^{2} Momentum Transfer",q2bins.size() - 1, &q2bins[0]); //etc etc etc
    //h1["dis_elec_angle"] = new TH1F("dis_elec_angle","Opening Angle between the Beam and Scattered Electron",phibins.size() - 1, &phibins[0]); //etc etc etc
