...
bool MyAnalysis::ReadPODIO()
{
    ...
    //AW 20260616
    //Trying to look at clusters from B0 and the ZDC!!
    auto& clusters_B0ECal = frame.get<edm4eic::ClusterCollection>("B0ECalClusters"); // B0 clusters!!
    auto& clusters_ZDC = frame.get<edm4eic::ClusterCollection>("EcalFarForwardZDCClusters"); // ZDC clusters!!!
    ...
    //AW 20260616 want to duplicate the structure!!!
    auto mapB0 = cluster_mapping(frame.get<edm4eic::MCRecoClusterParticleAssociationCollection>("B0ECalClusterAssociations"));
    auto mapZDC = cluster_mapping(frame.get<edm4eic::MCRecoClusterParticleAssociationCollection>("EcalFarForwardZDCClusterAssociations"));
    // ##########################
    ...
    //AW 20260616
    //add clusters to the event class!!!
    for (auto cluster : clusters_B0ECal) ev->bdt_clusters.push_back(cluster);
    for (auto cluster : clusters_ZDC) ev->bdt_clusters.push_back(cluster);
    // #################################
    ...
    for (auto asso : associations)
    {
        ...
        //AW 20260616
        //trying to duplicate the structure??
        par.bdt_clusters.clear();
        if (mapZDC.count(idsim)) par.bdt_clusters.insert(par.bdt_clusters.end(), mapZDC.at(idsim).begin(), mapZDC.at(idsim).end());
        if (mapB0.count(idsim)) par.bdt_clusters.insert(par.bdt_clusters.end(), mapB0.at(idsim).begin(), mapB0.at(idsim).end());
        // ###################
